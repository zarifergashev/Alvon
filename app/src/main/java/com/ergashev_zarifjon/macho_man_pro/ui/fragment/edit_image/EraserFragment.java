...
import com.ergashev_zarifjon.macho_man_pro.brush_view.BrushImageView;
import com.ergashev_zarifjon.macho_man_pro.brush_view.IPhotoSaveHelper;
import com.ergashev_zarifjon.macho_man_pro.brush_view.TouchImageView;
...
public class EraserFragment extends EIBaseFragment {
    ...
    private IPhotoSaveHelper saveHelper;

    private boolean isImageResized;
    private MediaScannerConnection msConn;
    private int MODE;

    public void setSaveHelper(IPhotoSaveHelper saveHelper) {
    ...
    public static Fragment newInstance(Bitmap image, IPhotoSaveHelper saveHelper) {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.eraser_activity, container, false);
    }
   ...
   /*     if (getIntent() != null) {
            int imageres = getIntent().getIntExtra("IMAGE_RES_ID", -1);
            if (imageres != -1) {
                originalBitmap = BitmapFactory.decodeResource(getResources(), imageres);
                setBitMap();
                updateBrush((float) (mainViewSize.x / 2), (float) (mainViewSize.y / 2));
            } else {
                String imagePath = getIntent().getStringExtra("IMAGE_PATH");
                if (imagePath != null) {
                    readTempImage(imagePath, bitmap -> {
                        originalBitmap = bitmap;
                        setBitMap();
                        updateBrush((float) (mainViewSize.x / 2), (float) (mainViewSize.y / 2));
                        return null;
    ...
    public void initViews(View view) {
        touchImageView = view.findViewById(R.id.drawingImageView);
        brushImageView = view.findViewById(R.id.brushContainingView);
        llTopBar = view.findViewById(R.id.ll_top_bar);
        rlImageViewContainer = view.findViewById(R.id.rl_image_view_container);
        ivUndo = view.findViewById(R.id.iv_undo);
        ivRedo = view.findViewById(R.id.iv_redo);
        ivDone = view.findViewById(R.id.iv_done);
        sbOffset = view.findViewById(R.id.sb_offset);
   ...
   /*     if (lastEditedBitmap != null) {
    ...
    private void saveImage() {
        ...
        if (saveHelper != null) {
            saveHelper.imageSave(highResolutionOutput);
        }
        new Handler().postDelayed(() -> {
            getListener().onBackPress();
        }, 200);
    ...
    private class imageSaveByAsync extends AsyncTask<String, Void, Boolean> {
        ...
        protected void onPreExecute() {
            if (getActivity() != null) getActivity().getWindow().setFlags(16, 16);
        }
        ...
        protected void onPostExecute(Boolean success) {
            if (getActivity() != null) {
                Toast toast = Toast.makeText(getActivity().getBaseContext(), "PNG Saved", Toast.LENGTH_LONG);
    ...
    public void scanPhoto(String imageFileName) {
        this.msConn = new MediaScannerConnection(getActivity(), new ScanPhotoConnection(imageFileName));
        this.msConn.connect();
    }
